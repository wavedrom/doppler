...
const progress = (lane, desc, pstate, tStart, tFinish) => {
    ...
    signals.map(sig => {
      ...
      if (t === cursor) {
        ...
        if (le) {
          ...
          const brick = {name: sig.name, label: sig.label, t, tail};
          pco[pc].bricks.push(brick);
          // console.log(brick);
        }
...

  {
    ...
    for (const iClock of clockEr) {
      ...
      if (vClock) {
        for (const [i, key] of Object.keys(othero).entries()) {
          const valid = othero[key].valid.sampler.next(tClock).value;
          const addr = othero[key].addr.sampler.next(tClock).value;
          if (valid) {
            pushBrick(listing, pco, addr, 'pc' + i, '', tClock);
