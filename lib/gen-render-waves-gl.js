...
const genRenderWavesGL = (els) => {
  ...
  els.view0.replaceChildren(cnvs);
  // cnvs.width = 1024;
  // cnvs.height = 1024;
  ...
  return desc => {
    ...
    return (pstate, plugins) => {
      ...
      return () => {
        ...
        gl.uniform2f(loco.scale, 2 * xScale / width, yStep * yDuty / cHeight / 2); // FIXME 40
        ...
        desc.view.map((lane, idx) => {
          ...
          if (chang !== undefined) {
            ...
            gl.uniform2f(loco.offset,
              (2 * xOffset / width) - 1,
              (2 * yOffset - yStep * (idx + .7)) / cHeight + 1
            );
            gl.drawArrays(
              gl.LINE_STRIP, // mode
              0, // first
        ...
        ['xOffset', 'yOffset', 'xScale'].map(key => {
          localStorage.setItem('pstate.' + key, pstate[key]);
        });
