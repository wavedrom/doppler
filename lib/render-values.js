...
const renderValues = function* (desc, pstate) {
  ...
  for (let i = 0; i < (iskip + ilen); i++) {
    ...
    if (lane && lane.kind === 'DIZ') {
      markers.push(['g', tt(0, Math.round((i - (iskip - ifirst) + 2) * yStep / 2))].concat(water(lane, desc, pstate)));
    } else

    if (lane && lane.kind === 'brace') {
      markers.push(['g', tt(0, Math.round((i - (iskip - ifirst) + 2) * yStep / 2))].concat(bracer(lane, desc, pstate)));
    } else
