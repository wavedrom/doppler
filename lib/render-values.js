...
const renderValues = function* (desc, pstate) {
  ...
  const ilen = 2 * height / yStep; // FIXME 50
  // const cHeight = height - 40;
  const iskip = 2 * yOffset / yStep;
  ...
  for (let i = 0; i < (iskip + ilen); i++) {
    ...
    if (lane && lane.kind === 'DIZ') {
      markers.push(['g', tt(0, Math.round((i - (iskip - ifirst) + 1.18) * yStep / 2))].concat(water(lane, desc, pstate)));
    } else

    if (lane && lane.kind === 'brace') {
      markers.push(['g', tt(0, Math.round((i - (iskip - ifirst) + 1.18) * yStep / 2))].concat(bracer(lane, desc, pstate)));
    } else

    if (lane && lane.ref) {
      const chango = desc.chango[lane.ref];
      if (chango && chango.kind === 'vec') {
        const mLane = ['g', tt(0, Math.round((i - (iskip - ifirst) + .9) * yStep / 2))];
        const { wave } = chango;
