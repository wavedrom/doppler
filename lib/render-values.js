...
const renderValues = function* (desc, pstate) {
  ...
  let i0;
  for (i0 = 0; i0 < ilen; i0 += 1) {
    const lane = desc.view[i0];
    if (lane && (lane.name || lane.kind) && (i0 > iskip)) {
      i0 -= 1;
      break;
    }
  }
  ...
  for (let i = 0; i < ilen; i++) {
    ...
    if (lane && lane.kind && lane.kind === 'DIZ') {
      ml.push(['g', tt(0, Math.round((i - (iskip - i0) + 2) * yStep / 2))].concat(water(lane, desc, pstate)));
    } else
    if (lane && lane.ref) {
      const chango = desc.chango[lane.ref];
      if (chango && chango.kind === 'vec') {
        const mLane = ['g', tt(0, Math.round((i - (iskip - i0) + 1.75) * yStep / 2))];
        const { wave } = chango;
